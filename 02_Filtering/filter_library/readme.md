...
| Sampling Frequency | Pour un facteur de décimation de $`D`$ <br />$`fs_{PDM} = D * fs_{PCM}`$  | Cette valeur correspond a la fréquence d'échantillonnage ($`fs_{PDM}`$) </br> du signal PDM
