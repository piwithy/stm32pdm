...
Le signal PCM recherché doit sera échantillonné à $`48 kHz`$ ce qui nous donne une fréquence de coupure maximum de $`24 kHz`$ afin de se donner de la marge on vas utilisé une fréquence de coupure a $`20 kHz`$, cette limitation dans la bande passante n'est pas un problème car la fréquence maximum qui peut etre perçus par l'oreille humaine est au voisinage de $`20 kHz`$  

 2. A quelle fréquence est échantillonné notre signal PDM ?

Dans cette exemple voulons un signal PCM a $`48 kHz`$, qui serat issue du filtre avec un facteur de décimation $`D`$ de 64. Le signal PDM sera donc échantillonné à $`3.072 MHz`$ ($`fs_{PDM}= D * fs_{PCM} = 64 * 48 * 10^{3} = 3.072 * 10^{6}`$)
...
# cf. 01_PCM_PDM_Toolbox pour l'installation de l'environement virtuel
source venv/bin/activate
python 02_Filtering/filter_library/generate_fir_filter.py -t 16 -s 3072000 -c 20000 -b 30 -p
```

l'option `-p` trace la courbe de réponse en fréquence du filtre dasn notre cas elle est la suivante:
...
L'intégration de la bibliothèque dans un Projet CMAKE "*classique*" se fait en plusieurs etapes:
 ...
 2. Dans le `CMakeLists.txt` ajouter à l'instruction `include_directories` le dossier contenant la librairie
 3. Dans le `CMakeLists.txt` ajouter a l'instruction `add_executable` le fichier `pdm_fir.c`
...
On retrouvera le fichier `CMakeLists.txt` suivant une fois la librairie intégré:
```cmake
...
L'integration de la librairie a un projet Clion pour STM32CubeMX (cf. [STM32CubeMX projects](https://www.jetbrains.com/help/clion/embedded-development.html)) est un peu plus complexe car le fichier `CMakeLists.txt` est généré automatiquement par Clion à chaque génération du code par CubeMX. Il faut donc opérer nos modification dans un fichier nommé `CMakeLists_template.txt`. Dans ce cas il faut suivre les étapes suivantes:
 ...
 3. Regenerer les sources a partir de CubeMX (pour rafraichir le projet dans Clion et regénérer le `CMakeLists.txt`)
