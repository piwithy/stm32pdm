...
def main():
    ...
    argument_parser.add_argument('-c', '--coefs', type=float, help="Fourier Coefficients", nargs='+')
    ...
    coefs = args.coefs if args.coefs else [1.]
    N = int(T * fs)

    t = np.linspace(0, T, N)
    x = np.zeros(t.shape)
    for i in range(len(coefs)):
        x += coefs[i] * np.sin(2 * np.pi * (i + 1) * f0 * t)
    ...
    plt.title("Generated PCM Signal\nfs={:.1f}kHz f0={}Hz, \nFourier coefficients: {}".format(fs / 1000, f0, coefs))
